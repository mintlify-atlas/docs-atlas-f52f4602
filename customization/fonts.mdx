---
title: "Typography and Font System in Retro Cosmic Arcade"
sidebarTitle: "Fonts"
...
<Steps>
  ...
  <Step title="Update the @import URL">
    Open `assets/main.css` and edit the `@import` string. Replace the old `family=` parameter with the new one:
  ...
  <Step title="Update the Tailwind config">
    Open `tailwind.config.js` and update the matching entry in `theme.extend.fontFamily`:
  ...
  <Step title="Check sizes and line-height">
    Different fonts have different x-heights and advance widths. After swapping, scan for any components that used size-specific classes (`text-[10px]`, `leading-tight`, etc.) tuned to the old font and adjust as needed.
  </Step>
</Steps>
