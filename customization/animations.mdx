---
title: "CSS Animations and Framer Motion — Retro Cosmic Arcade"
sidebarTitle: "Animations"
description: "CSS keyframe animations and Framer Motion power every hover, drag, and particle effect in Retro Cosmic Arcade. Learn how to customize or disable them."
---
...
Two custom keyframe animations are defined in `assets/main.css` and compiled into Tailwind utility classes (`animate-blink` and `animate-marquee`) via the `theme.extend.animation` and `theme.extend.keyframes` source config blocks. Because the project ships as a compiled SPA, modifying these animations requires editing the source config and rebuilding with `vite build`.
