---
title: "Footer: CRT Scanline Toggle and Retro Badges"
sidebarTitle: "Footer"
...
When `scanlines-active` is present on `<body>`, the `.scanlines` utility class drives a fixed, full-viewport `<div>` overlay (rendered inside the footer at `z-40`) that produces the retro scan-line effect.
...
<Steps>
  <Step title="Page loads">
    `scanlines` initialises as `true`. `useEffect` fires and adds `scanlines-active` to `document.body`. The fixed `.scanlines` div is rendered inside the footer.
  </Step>
  <Step title="User clicks the toggle button">
    `setScanlines(!scanlines)` flips the state. `useEffect` re-runs, removing `scanlines-active` from `document.body`. The overlay div is no longer rendered.
  </Step>
